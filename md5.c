 ...
 * 
 * Copyright (c) 2008 Marko Kreen, Skype
 * 
...
static void
md5_update(struct md5_ctx *ctx, const void *data, unsigned int len)
{
	unsigned int n;
...
static void
md5_final(struct md5_ctx *ctx, uint64_t *dst)
{
	...
	/* result in little-endian uint64s */
	dst[0] = ((uint64_t)ctx->b << 32) | ctx->a;
	dst[1] = ((uint64_t)ctx->d << 32) | ctx->c;
}
